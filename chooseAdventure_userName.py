...
def main(session):
    ...
    topic_content_1 = ('topic: ~storySetup()\n'
                       ...
                       '''proposal: %Choices Would you like to hear about ^enumerate(~stories)\n
                        u1:(Hood) Great! ^topicTagReactivate(RidingHood,who)
                        u1:(Arthur) Awesome
                        u1:(Gordon) Cool'''
    ...
    topic_content_2 = ('topic: ~RidingHood()\n'
                       ...
                       u:({"My name is" "Call me"} _*) Ok $1 $Name=$1 ^gotoReactivate(info2)'''
                       ...
                       ["Does she $Name=='Little Red Ridding Hood'" "Do you"] stop to smell the flowers and have a snack, or coninue along the path?\n
                       u1:(Rest) Ok $Pronoun decides to rest ^gotoReactivate(RidingHood,startRest)
                       u1:(Continue) Ok $Pronoun decides to continue ^gotoReactivate(startContinue)'''
                       
        #If varaibles work need to continue implementation below this point               
